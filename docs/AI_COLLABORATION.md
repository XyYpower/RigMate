...
>
> 最后更新：2026-09-24 ｜ 当前阶段：**M34 体验基线完成**——目标入口、方案主结果、持续可见的自然语言修订输入、面向新手的硬件类别查询与真实目录检索已统一到仪器白工作台；21 E2E + 166 单测全绿，lint/typecheck/build 通过。M35–M39 路线见 `docs/superpowers/specs/2026-09-24-v2-product-design.md`。
> **换窗口交接：先读 §0 交接快照。**
...
- **下一项工作**：M35 工作台接线：统一 `DesignWorkspaceView` 视图契约、旧版方案只读浏览、依据入口、追问与失败恢复；保持 M34 的输入/结果层级和现有 DIY/报告功能不回退。随后 M36 做目录与证据质量，M37 做受约束的模型选件，M38 部署与完整闭环，M39 公开试用。详细范围与退出条件见新设计文档。
...
| 目标驱动垂直切片：/ 目标入口 + /design/[id] 审阅 + 接受进 DIY（v6 迁移五表） | ✅ 完成（M28，规则式生成） |
...
1. **M34 体验基线**：✅ 完成。新增 `src/ui/v2-workspace.css`；方案页调整输入从侧栏移到主结果下方；硬件资料页先讲类别用途再检索；保留既有可访问名称以避免回归。真实浏览器检查首页、方案页和硬件页；桌面与窄屏规则均保留。
2. **M35 工作台接线**：统一视图契约，提供旧版方案只读浏览、来源入口、追问与失败恢复；保持现有 DIY/报告功能不回退。
