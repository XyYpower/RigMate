...
>
> 最后更新：2026-09-23 ｜ 当前阶段：**M31 自然语言修订方案已上线**——方案页侧栏直接说"预算压到 1.8 万 / 我已有电源"即可生成新版本方案（LLM 优先、规则兜底、理解不了诚实追问），旧版本保留可追溯。此前 M27-M30：目标驱动垂直切片 + 自有规格库 + LLM 意图解析。下一步 M32 DIY 信息渐进披露。
> **换窗口交接：先读 §0 交接快照。**
...
- **测试基线**：162 单测 + 19 E2E 全绿；lint / typecheck / build 通过。
- **本机注意事项**：
...
- **下一项工作（M32）**：DIY 信息渐进披露（方案页→DIY 的衔接打磨：规则细节/字段完整度/检查历史折叠呈现）+ 方案版本差异对比（第 N 版 vs 第 N-1 版改了哪些件）；之后 M33 产品化收口（真实浏览器多状态验收 + 部署准备）。
- **前端所有权**：归 AI 窗口（全栈）。
